...
const Editor = () => {
  ...
  useEffect(() => {
    posthog.onFeatureFlags(() => {
      if (posthog.isFeatureEnabled('show-advanced-gates')) {
        setShowAdvancedGates(true);
      } else {
    ...
    if (isSimulating) {
      ...
      try {
        ...
        syncIntervalRef.current = setInterval(() => {
          ...
          const { visualNodesValues, activeEdges, visualMacroOutputs } =
            simulatorRef.current.getVisualState();
          ...
          setNodes((currentNodes) =>
            currentNodes.map((n) => {
              ...
              if (visualMacroOutputs[n.id]) {
                const currentOutputsStr = JSON.stringify(
                  n.data.macroOutputs || {},
                );
              ...
              if (
                visualNodesValues[n.id] !== undefined &&
                visualNodesValues[n.id] !== newData.value
              ) {
  ...
  const recompileIfSimulating = useCallback(
    (newNodes, newEdges) => {
      if (isSimulating) {
        try {
          simulatorRef.current = new Simulator(
  ...
  const handleMacroAction = useCallback(
    (savedCircuit) => {
      if (isReplaceMode && switchTargetId) {
        // РЕЖИМ ЗАМІНИ
        const inputsCount = savedCircuit.data.nodes.filter(
          (n) => n.type === 'switch',
        ).length;
        const outputsCount = savedCircuit.data.nodes.filter(
          (n) => n.type === 'bulb',
        ).length;
  ...
  useEffect(() => {
    useCircuitStore.setState({
      ...
      updateNodeLabel,
      tickClock,
      updateNodeData: updateNodeDataLocal,
    });
    ...
    <div className='min-h-screen flex flex-col h-screen'>
      <nav className='bg-slate-800 border-b border-slate-700 px-4 py-3 flex justify-between items-center z-10 shrink-0'>
        <div className='flex items-center gap-4'>
          ...
          {isRenaming ? (
            <div className='flex items-center gap-2 bg-slate-800 p-1 rounded border border-slate-600'>
              ...
              {isMacro && (
                <select
                  ...
                  className='bg-slate-700 text-xs text-white rounded p-1'
                >
              ...
              <button
                ...
                className='bg-blue-600 text-xs px-2 py-1 rounded hover:bg-blue-500 text-white'
              >
                Save
              </button>
            </div>
            ...
            <div
              className='flex flex-col cursor-text hover:bg-slate-700 px-2 py-1 rounded transition'
            ...
            >
              ...
              <span className='text-[10px] text-slate-400'>
                {currentCircuit?.isMacro
                  ? `Macro (${currentCircuit.category})`
                  : 'Private Circuit'}
              </span>
            </div>
          )}
      ...
      <main className='flex-1 w-full h-full bg-slate-900 relative'>
        ...
        >
          ...
          <Panel
            position='top-center'
            className='bg-slate-800 p-1.5 rounded-lg border border-slate-700 shadow-xl flex gap-1 mt-2'
          >
            <button
              onClick={handleUndo}
              disabled={!canUndo}
        ...
        >
          {nodes.find((n) => n.id === contextMenu.id)?.type === 'macro' && (
            <>
              ...
              >
                Replace Template{' '}
                <span className='text-[10px] bg-blue-900/50 px-1 rounded'>
                  Swap
                </span>
              </button>
              <div className='h-px bg-slate-700 my-1'></div>
      ...
      <dialog
        id='macro-modal'
        className='bg-slate-800 border border-slate-600 rounded-xl shadow-2xl p-0 backdrop:bg-black/60 backdrop:backdrop-blur-sm w-[500px]'
      >
        <div className='p-4 border-b border-slate-700 flex justify-between items-center'>
          ...
          <button
            onClick={() => document.getElementById('macro-modal').close()}
        ...
        <div className='p-4 max-h-[400px] overflow-y-auto'>
          {['Arithmetic', 'Memory', 'Custom'].map((cat) => {
            const catCircuits = circuits.filter(
              (c) => c._id !== id && c.isMacro && c.category === cat,
            );
                ...
                <div className='grid grid-cols-2 gap-2'>
                  {catCircuits.map((circuit) => (
                    ...
                    >
                      ...
                      <span className='text-[9px] bg-blue-900/50 text-blue-300 px-1.5 rounded border border-blue-800'>
          ...
          {circuits.filter((c) => c.isMacro && c._id !== id).length === 0 && (
            <div className='text-center text-slate-500 py-8'>
              No macros published yet. <br /> Edit settings of your circuits to
              publish them.
            </div>
          )}
        </div>
