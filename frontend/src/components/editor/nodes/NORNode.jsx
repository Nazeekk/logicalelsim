...
const NORNode = ({ data, selected }) => {
  ...
  return (
    <div className={`relative w-20 h-16 border-2 flex items-center justify-center transition-all duration-200 bg-slate-800 shadow-lg
      ${isActive ? 'border-pink-400 shadow-[0_0_15px_rgba(244,114,182,0.4)]' : 'border-slate-600'}
    ...
    >
      <span className={`font-bold text-sm select-none ${isActive ? 'text-pink-400' : 'text-slate-400'}`}>
        NOR
      </span>

      <div className={`absolute -right-[6px] w-[10px] h-[10px] rounded-full border-2 bg-slate-800 
        ${isActive ? 'border-pink-400' : 'border-slate-600'}`}></div>
